...
def _normalize_scores(results: Dict[str, SearchResult]) -> None:
    ...
    Uses min-max normalization within each score type.
    """
    if not results:
        return
    ...
    if bm25_scores:
        ...
        if range_bm25 > 0:
            ...
            # All scores are equal, normalize to 1
            for result in results.values():
                if result.bm25_score > 0:
...
def _blend_scores(
    ...
    for result in results.values():
        ...
        if has_vector and has_bm25:
            ...
            result.final_score = result.vector_score * (vector_weight + bm25_weight * 0.5)
        elif has_bm25:
...
def _hydrate_results(session: Session, results: List[SearchResult]) -> None:
    ...
    if by_type["snapshot"]:
        ...
        snapshots = {s.id: s for s in session.query(Snapshot).filter(Snapshot.id.in_(snapshot_ids)).all()}
    ...
    if by_type["episode"]:
        ...
        episodes = {e.id: e for e in session.query(Episode).filter(Episode.id.in_(episode_ids)).all()}
    ...
    if by_type["memory"]:
        ...
        memories = {m.id: m for m in session.query(Memory).filter(Memory.id.in_(memory_ids)).all()}
